        ...
        return (
            <View style={styles.container}>

                {
                     ...
                     renderItem={({item}) => {
                         return (
                             <View style={{elevation:8, marginBottom:10,borderRadius:10, backgroundColor:'#575FCF', padding:10}}>
                                 <Text style={{fontSize:20,  fontWeight:'bold', color:'#fff', marginBottom: 10,}} > {item.title}</Text>
                                 <Text style={{fontSize:17, lineHeight:30, color:'#fff'}}>{item.content}</Text>
                                  <View style={{flexDirection:'row', justifyContent:'flex-end', marginTop:25}}>
                                      <TouchableHighlight onPress={() => this.props.navigation.navigate('Edit',{...item})}>
                                              <View style={{marginRight:15}}>
                                                  <Icon size={20} color="white" name="edit" />
                                              </View>
                                      </TouchableHighlight>   
